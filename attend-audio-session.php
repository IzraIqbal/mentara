...
<html>
<head>
    <title>Audio Session - Therapist</title>
    <style>
        body {
...
<body>
    ...
    <h2>Therapist - Audio Session</h2>

    <div class="session-info">
        <p><strong>Session Date:</strong> <?= htmlspecialchars($session_date) ?></p>
        <p><strong>Client ID:</strong> <?= $client_id ?> | <strong>Therapist ID:</strong> <?= $user_id ?></p>
    ...
    <div class="call-panel">
        <div class="users">
            <div class="user">
                ...
                <div id="clientStatus" class="status disconnected">Client: Waiting...</div>
            </div>
            <div class="user">
                <img src="images/therapist_audio.png" alt="Therapist Icon">
                <div id="therapistStatus" class="status disconnected">You: Not Connected</div>
            </div>
        </div>
    ...
    <script>
        ...
        joinBtn.onclick = () => {
            fetch("audio_join.php", {
                ...
                therapistStatus.textContent = "You: Connected";
                therapistStatus.className = "status connected";
            });
        };
        ...
        leaveBtn.onclick = () => {
            fetch("audio_leave.php", {
                ...
                therapistStatus.textContent = "You: Disconnected";
                therapistStatus.className = "status disconnected";
            });
        };
        ...
        setInterval(() => {
            ...
            .then(data => {
                if (data.status === 'connected') {
                    clientStatus.textContent = "Client: Connected";
                    clientStatus.className = "status connected";
                } else {
                    clientStatus.textContent = "Client: Waiting...";
        ...
        setInterval(async () => {
            const res = await fetch(`receive_voice_clip.php?session_id=${sessionId}&role=${otherRole}`);
